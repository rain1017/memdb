...
proto.insert = function(connId, id, doc){
	...
	return Q.fcall(function(){
		...
		logger.debug('connection[%s].collection[%s].insert(%s, %s) => %s', connId, self.name, id, util.inspect(doc), ret);
		return ret;
	});
};
...
proto.find = function(connId, id, fields){
	...
	return Q.fcall(function(){
		...
		logger.debug('connection[%s].collection[%s].find(%s, %s) => %s', connId, self.name, id, fields, util.inspect(ret));
		return ret;
	});
};
...
proto.update = function(connId, id, doc, opts){
	...
	return Q.fcall(function(){
		...
		logger.debug('connection[%s].collection[%s].update(%s, %s, %s) => %s', connId, self.name, id, util.inspect(doc), opts, ret);
		return ret;
	});
};
...
proto.findByIndex = function(connId, field, value, fields){
	...
	.then(function(ids){
		...
		.then(function(ret){
			logger.debug('connection[%s].collection[%s].findByIndex(%s, %s, %s) => %s', connId, self.name, field, value, fields, util.inspect(ret));
			return ret;
		});
	});
};
