...
function NativeCollection () {
	...
	this.id = uuid.v4();
	//logger.debug('create nativecollection for table: %s, ', this.name, new Error().stack);
}
...
NativeCollection.prototype.findOneInMdb = function(id, cb) {
	logger.info('findOneInMdb %s: %j', this.name, arguments);
...
NativeCollection.prototype.insert = function(docs, opts, cb) {
	logger.info('insert %s: %j', this.name, arguments);
...
NativeCollection.prototype.remove = function(selector, opts, cb) {
	logger.info('remove %s: %j', this.name, arguments);
...
NativeCollection.prototype.update = function(selector, doc, opts, cb) {
	logger.info('update %s: %j', this.name, arguments);
...
NativeCollection.prototype.save = function(doc, opts, cb) {
	logger.info('save %s: %j', this.name, arguments);
...
for (var i in Collection.prototype) {
	...
	(function(i){
		...
		NativeCollection.prototype[funcName] = function () {
			...
			logger.debug('nativecollection[%s][%s] %s: ', this.id, this.name, funcName, arguments);
			if (debug) {
