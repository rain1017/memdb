...
NativeCollection.prototype.findMdb = function(id, cb) {
	logger.debug('findMdb %s: %j', this.name, arguments);
...
NativeCollection.prototype.findForUpdate = function(id, cb) {
	logger.debug('findForUpdate %s: %j', this.name, arguments);
	var conn = mdb.autoConnect();
	var coll = conn.collection(this.name);
	Q.fcall(function(){
		return coll.findForUpdate(id);
	}).nodeify(cb);
};

NativeCollection.prototype.findByIndex = function(field, value, cb){
	logger.debug('findByIndex %s: %j', this.name, arguments);
	var conn = mdb.autoConnect();
	var coll = conn.collection(this.name);
	Q.fcall(function(){
