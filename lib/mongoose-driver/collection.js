...
var MongooseCollection = require('mongoose/lib/collection')
	...
	, memorydb = require('../index')
	, logger = require('pomelo-logger').getLogger('memorydb-client', __filename)
	, Q = require('q');
...
NativeCollection.prototype.findMdb = function(id, cb) {
	var self = this;
	return Q.fcall(function(){
		return memorydb.goose.autoConnect();
	})
	.then(function(conn){
		return conn.collection(self.name).find(id);
	})
	.nodeify(cb);
};

NativeCollection.prototype.findForUpdate = function(id, cb) {
	var self = this;
	return Q.fcall(function(){
		return memorydb.goose.autoConnect();
	})
	.then(function(conn){
		return conn.collection(self.name).findForUpdate(id);
	})
	.nodeify(cb);
};

NativeCollection.prototype.findByIndex = function(field, value, cb){
	var self = this;
	return Q.fcall(function(){
		return memorydb.goose.autoConnect();
	})
	.then(function(conn){
		return conn.collection(self.name).findByIndex(field, value);
	})
	.nodeify(cb);
};

NativeCollection.prototype.findCached = function(id, cb){
	var self = this;
	return Q.fcall(function(){
		return memorydb.goose.autoConnect();
	})
	.then(function(conn){
		return conn.collection(self.name).findCached(id);
	})
	.nodeify(cb);
};
...
NativeCollection.prototype.insert = function(docs, opts, cb) {
	...
	.then(function(conn){
		var coll = conn.collection(self.name);
...
NativeCollection.prototype.remove = function(selector, opts, cb) {
	...
	.then(function(conn){
		return conn.collection(self.name).remove(selector._id);
	})
	.nodeify(cb);
};
...
NativeCollection.prototype.update = function(selector, doc, opts, cb) {
	...
	.then(function(conn){
		return conn.collection(self.name).update(selector._id, doc['$set']);
	})
	.nodeify(cb);
};
