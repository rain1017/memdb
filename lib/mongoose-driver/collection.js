...
var MongooseCollection = require('mongoose/lib/collection')
	...
	, memdb = require('../index')
	, logger = require('pomelo-logger').getLogger('memdb-client', __filename)
	, P = require('bluebird');
...
NativeCollection.prototype.findMdb = function(query, cb) {
	var self = this;
	return P.try(function(){
		return memdb.goose.autoConnect();
	})
	.then(function(conn){
		return conn.collection(self.name).find(query);
	})
	.nodeify(cb);
};

NativeCollection.prototype.findOneMdb = function(query, cb){
	var self = this;
	return P.try(function(){
		return memdb.goose.autoConnect();
	})
	.then(function(conn){
		return conn.collection(self.name).findOne(query);
	})
	.nodeify(cb);
};

NativeCollection.prototype.findByIdMdb = function(id, cb) {
	var self = this;
	return P.try(function(){
		return memdb.goose.autoConnect();
	})
	.then(function(conn){
		return conn.collection(self.name).findById(id);
	})
	.nodeify(cb);
};

NativeCollection.prototype.findForUpdate = function(query, cb) {
	var self = this;
	return P.try(function(){
		return memdb.goose.autoConnect();
	})
	.then(function(conn){
		return conn.collection(self.name).findForUpdate(query);
	})
	.nodeify(cb);
};

NativeCollection.prototype.findOneForUpdate = function(query, cb) {
	var self = this;
	return P.try(function(){
		return memdb.goose.autoConnect();
	})
	.then(function(conn){
		return conn.collection(self.name).findOneForUpdate(query);
	})
	.nodeify(cb);
};

NativeCollection.prototype.findByIdForUpdate = function(id, cb) {
	var self = this;
	return P.try(function(){
		return memdb.goose.autoConnect();
	})
	.then(function(conn){
		return conn.collection(self.name).findByIdForUpdate(id);
	})
	.nodeify(cb);
};

NativeCollection.prototype.findByIdCached = function(id, cb){
	var self = this;
	return P.try(function(){
		return memdb.goose.autoConnect();
	})
	.then(function(conn){
		return conn.collection(self.name).findByIdCached(id);
	})
	.nodeify(cb);
};
...
NativeCollection.prototype.insert = function(docs, opts, cb) {
	...
	.then(function(conn){
		return conn.collection(self.name).insert(docs);
	})
	.nodeify(cb);
};
...
NativeCollection.prototype.remove = function(selector, opts, cb) {
	...
	.then(function(conn){
		return conn.collection(self.name).remove(selector, opts);
	})
	.nodeify(cb);
};
...
NativeCollection.prototype.update = function(selector, doc, opts, cb) {
	...
	.then(function(conn){
		return conn.collection(self.name).update(selector, doc, opts);
	})
	.nodeify(cb);
};
