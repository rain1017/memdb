...
mongoose.transaction = function(func){
	var self = this;
	return P.try(function(){
		return self.autoConnect();
	})
	.then(function(conn){
		return conn.transaction(func);
	});
};
...
['find', 'findOne', 'findById', 'findLocked', 'findOneLocked', 'findByIdLocked', 'findCached'].forEach(function(method){
	Model[method] = function(){
		...
		.then(function(conn){
			var coll = conn.collection(self.collection.name);
