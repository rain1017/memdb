 ...
 * (Deprecated)
 *
 * Force unlock if the holder has no heartbeat
 * return null if unlocked, otherwise return holder's shardId
 */
proto.autoUnlock = function(docId){
	logger.warn('autoUnlock is deprecated');
	...
	.then(function(ret){
		if(ret && ret[0] === 1){
			commited = true;
			logger.warn('autoUnlocked %s', docId);
			return null;
		}
		else{
			return shardId;
