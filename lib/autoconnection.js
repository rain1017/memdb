...
proto.openConnection = function(){
    ...
    .then(function(connId){
        ...
        logger.info('[conn:%s] open connection', connId);

        this.freeConnections[connId] = true;

        var self = this;
        conn.on('close', function(){
            logger.info('[conn:%s] connection closed', connId);
...
proto._runTask = function(){
    ...
    var self = this;
    scope.run(function(){
        logger.info('[conn:%s] transaction start', connId);
        ...
        return promise.then(function(ret){
            ...
            .then(function(){
                logger.info('[conn:%s] transaction done (%sms)', connId, Date.now() - startTick);
            ...
            .then(function(){
                logger.error('[conn:%s] transaction error %s', connId, err.stack);
                delete scope.__memdb__;
