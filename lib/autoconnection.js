...
proto.openConnection = function(shardId){
    ...
    if(this.openConnectionLock.isBusy(shardId)){
        return;
    }
...
proto._runTransaction = function(func, conn, shardId){
    ...
    scope.__memdb__ = {shard: shardId, conn: conn.connId, trans : uuid.v4()};

    var self = this;
    scope.run(function(){
        logger.info('[shard:%s][conn:%s] transaction start', shardId, conn.connId);
        ...
        .then(function(ret){
            return conn.commit()
            .then(function(){
                logger.info('[shard:%s][conn:%s] transaction done (%sms)', shardId, conn.connId, Date.now() - startTick);
            ...
            .then(function(){
                logger.error('[shard:%s][conn:%s] transaction error %s', shardId, conn.connId, err.stack);
                delete scope.__memdb__;
                deferred.reject(err);
            });
        })
