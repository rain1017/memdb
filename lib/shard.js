...
proto.stop = function(force){
	...
	return Q.fcall(function(){
		if(!force){
			...
			// Force stop, clear data in slave
			return self.slave.clear();
		}
	})
	.then(function(){
	...
	.then(function(){
		...
		self.emit('stop');
		if(force){
			logger.error('shard[%s] is force stoped, you may lose some progress', self._id);
		}
		else{
...
proto.update = function(connectionId, key, doc, opts){
	...
	logger.debug('shard[%s].update(%s, %s, %j, %s) => %s', this._id, connectionId, key, doc, opts, ret);
	return ret;
};
